...
class KametiPermissionService {
  ...
  bool canManageKameti(MemberModel? member) =>
      member?.role == MemberRole.organizer ||
      member?.role == MemberRole.coOrganizer;
  bool canViewKameti(MemberModel? member) =>
      member != null && member.status != MemberStatus.removed;
  bool canSubmitPayment(MemberModel? member, String paymentMemberId) =>
      member != null &&
      member.id == paymentMemberId &&
  ...
  bool canSubmitBid(MemberModel? member) =>
      member != null &&
      member.status == MemberStatus.active &&
      !member.hasReceivedKameti;
