...
class SecurityCenterScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final members = kametiId.isEmpty
    ...
    final disputes = kametiId.isEmpty
        ? ref.watch(securityControllerProvider).disputes
        : ref
            .read(securityControllerProvider.notifier)
            .getDisputesByKametiId(kametiId);
    final logs = kametiId.isEmpty
        ? ref.watch(securityControllerProvider).auditLogs
        : ref
            .read(securityControllerProvider.notifier)
            .getAuditLogsByKametiId(kametiId);
    return Scaffold(
      appBar: AppBar(
          title:
              Text(kametiId.isEmpty ? 'Security Center' : 'Kameti Security')),
      body: SafeArea(
          ...
          if (user != null)
            Card(
              child: ListTile(
                ...
                subtitle: const Text(
                    'Security, privacy, disputes, reports, and trust score controls.'),
              ),
            ),
          SecurityWarningCard(
              message:
                  '${disputes.where((item) => item.status != dynamicResolved && item.status != dynamicClosed).length} unresolved dispute(s).'),
          SecurityWarningCard(
              message: '${risky.length} risky member(s) need review.'),
          Card(
            child: Column(children: [
              if (kametiId.isNotEmpty)
                ListTile(
                  ...
                  onTap: () => Navigator.of(context)
                      .pushNamed(AppRoutes.auditLogs, arguments: kametiId),
                ),
              if (kametiId.isNotEmpty)
                ListTile(
                  ...
                  trailing: const Icon(Icons.chevron_right),
                  onTap: () => Navigator.of(context)
                      .pushNamed(AppRoutes.disputes, arguments: kametiId),
                ),
              ListTile(
              ...
              ListTile(
                ...
                onTap: () => ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(
                        content: Text('Data export request prepared.'))),
              ),
              ListTile(
                leading: const Icon(Icons.delete_outline),
                title: const Text('Request Account Deletion'),
                subtitle: const Text(
                    'Financial records may remain for group integrity.'),
  ...
  void _requestDeletion(BuildContext context, WidgetRef ref) {
    ...
    ref.read(securityControllerProvider.notifier).createDeletionRequest(
        userId, 'User requested deletion from Security Center.');
    ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Deletion request created.')));
  }
}
