  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    if (selectedDraw == null) {
      return Scaffold(
          ...
          body: const Center(child: Text('Draw not found')));
    }
    final kameti =
        _findKameti(ref.watch(kametiControllerProvider), selectedDraw.kametiId);
    final memberController = ref.read(memberControllerProvider.notifier);
    final cycle = ref
        .read(paymentControllerProvider.notifier)
        .getCycle(selectedDraw.cycleId);
    final winner = memberController.getMember(selectedDraw.winnerMemberId);
    final eligibleMembers = selectedDraw.eligibleMemberIds
        .map(memberController.getMember)
        .whereType<MemberModel>()
        .toList();
    final excludedMembers = selectedDraw.excludedMemberIds
        .map(memberController.getMember)
        .whereType<MemberModel>()
        .toList();
        ...
        child: ListView(
          ...
          children: [
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _Line(
                        label: 'Cycle',
                        value:
                            'Month ${selectedDraw.cycleNumber} - ${cycle?.monthLabel ?? ''}'),
                    _Line(label: 'Winner', value: selectedDraw.winnerName),
                    _Line(label: 'Winner Phone', value: winner?.phone ?? '-'),
                    _Line(
                        label: 'Payout Amount',
                        value:
                            CurrencyFormatter.pkr(selectedDraw.payoutAmount)),
                    _Line(
                        label: 'Draw Date',
                        value: DateFormatter.display(selectedDraw.drawDate)),
                    _Line(
                        label: 'Draw Type', value: selectedDraw.drawType.label),
                    _Line(label: 'Created By', value: selectedDraw.createdBy),
                    _Line(
                        label: 'Eligible Members',
                        value: '${selectedDraw.totalEligibleMembers}'),
                    _Line(
                        label: 'Excluded Members',
                        value: '${selectedDraw.totalExcludedMembers}'),
                    if (selectedDraw.notes.isNotEmpty)
                      _Line(label: 'Notes', value: selectedDraw.notes),
                  ],
                ),
              ),
            ),
            ...
            ...eligibleMembers.map((member) => ListTile(
                title: Text(member.fullName), subtitle: Text(member.phone))),
            const SizedBox(height: 12),
